 ...
 __global__ void shade_kernel_sdf(
	 ...
	 switch (mode) {
		 ...
		 case ERenderMode::Shade: {
			 ...
			 vec3 suncol = vec3{255.f/255.0f, 225.f/255.0f, 195.f/255.0f} * 4.f * 1.f; // Distance encodes shadow occlusion. 0=occluded, 1=no shadow
		 ...
		 case ERenderMode::Lambert:
		 {
			 vec3 dir_color = vec3(1.f, 1.f, 1.f);
			 vec3 amb_color = vec3(1.f, 1.f, 1.f);
			 vec3 direct    = sun_dir;
			 direct = normalize(direct);
 
			 float dir_int = 0.6667f;
			 float amb_int  = 0.25f;
			 // printf("%f | %f %f %f | %f %f %f\n", dot(direct, normal), direct.x, direct.y, direct.z, normal.x, normal.y, normal.z);
			 color = dot(direct, normal);
		 }
	 }
